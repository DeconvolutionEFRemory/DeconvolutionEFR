...
if filterOn:
    file_handler_vel_bar     = XDMFFile(comm,'output/vel_leray_BDF2_pcd.xdmf')
    file_handler_press_bar   = XDMFFile(comm,'output/press_leray_BDF2_pcd.xdmf')
...
def lerayFilter(m_ns, indicator):
    u, _ = split(m_ns)
    leray_00  = + inner(D(ddv_bar), delta**2*indicator*D(dv_bar))*dx \
...
def localSensitivityDelta_NS(m_ns, m0_ns, m00_ns, m0_ns_delta, m00_ns_delta, m0_leray_delta, m00_leray_delta, dt):
...
## TODO: reuse the matrix from NS, only change assemble the L(rhs)
    ...
    rhs_delta = + 2./dt  * inner( ddv, rho * v0_delta ) * dx \
                ...
                - inner( ddv , rho * grad(v) * vf_delta_star) * dx  ## double check
...
## same functionSpace used for sensitivity as forward NS
    ...
    uf, _      = split(m_l)
    leray_00   = + inner(D(ddv_bar), delta**2 * indicator * 2*D(dv_bar)) * dx \
                 + inner(ddv_bar,dv_bar)*dx  

    leray_01   = - inner(div(ddv_bar),dp_bar)*dx 
    leray_10   = + inner(ddp_bar, div(dv_bar))*dx

    rhs_leray_delta  = + inner(ddv_bar,u_delta) * dx\
                       - inner(D(ddv_bar), 2* delta * indicator * 2*D(uf)) * dx\
                       - inner(D(ddv_bar), delta**2 * indicator_delta * 2*D(uf)) * dx\
                       - inner(D(ddv_bar), delta*2 * indicator * 2*D(uf)) * ds(3) \
                       - inner(D(ddv_bar), delta**2 * indicator_delta * 2*D(uf)) * ds(3) ## neumann bc (assume the NS has zero-stress on the outlet)
...
if filterOn:
    ...
    if sensitivityOn:
        ...
        F_delta = localSensitivityDelta_NS(m_ns, m0_ns, m00_ns, m0_ns_delta, m00_ns_delta, m0_leray_delta, m00_leray_delta, dt)
